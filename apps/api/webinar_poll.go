...
func (api WebinarPollAPI) PostWebinarPoll(c echo.Context) error {
	...
	tx.Where("webinar_site_id = ? and start_date <= ? and end_date >= ? ", payload.WebinarSiteId, payload.StartDate, payload.StartDate ).Find(&webinar_polls)
...
func (api WebinarPollAPI) GetWebinarPoll (c echo.Context) error  {
	...
	if tx.Preload("WebinarSite").
		...
		Where("idx = ? ", idx ).Find(webinar_poll).RecordNotFound() {

		return echo.NewHTTPError(http.StatusNotFound, "WEBINAR POLL QUESTION NOT FOUND")
	}

	return handler.APIResultHandler(c, true, http.StatusOK, webinar_poll)
}
...
func (api WebinarPollAPI) PutWebinarPoll (c echo.Context) error  {
	...
	webinar_polls := []rdbModel.WebinarPoll{}
	tx.Where(" start_date <= ? and end_date >= ? ", payload.StartDate, payload.EndDate ).Find(&webinar_polls)
